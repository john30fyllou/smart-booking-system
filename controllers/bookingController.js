...
const createBooking = (req, res) => {
    ...
    const { service_id, booking_date, start_time } = req.body;
    ...
    db.query(serviceSql, [service_id], (err, services) => {
        ...
        db.query(
            availabilitySql,
            [service.provider_id, booking_date, start_time, endTime],
            (err, availabilityResults) => {
                ...
                db.query(
                    conflictSql,
                    [service.provider_id, booking_date, endTime, start_time],
                    (err, conflictResults) => {
                        if (err) {
                        ...
                        db.query(
                            insertSql,
                            [customerId, service_id, booking_date, start_time, endTime],
                            (err, result) => {
                                if (err) {
...
const cancelBooking = (req, res) => {
    ...
    db.query(findBookingSql, [bookingId], (err, results) => {
        ...
        if (req.user.role !== 'admin' && booking.customer_id !== customerId) {
            return res.status(403).json({
                message: 'Access denied'
            });
        }
...
const getAvailableSlots = (req, res) => {
    ...
    db.query(serviceSql, [serviceId], (serviceError, serviceResults) => {
        if (serviceError) {
                ...
                db.query(
                    ...
                    (bookingsError, bookings) => {
                        ...
                        availabilityResults.forEach((availability) => {
                            let current = timeToMinutes(availability.start_time);
